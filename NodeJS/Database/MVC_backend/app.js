...
app.get('/folders/:id', (req, res) => {
    ...
    connection.query(`select * from folders where id='${Id}'`, (err, rows) => {
        if (err)
            throw (err);
        else {
...
app.post('/add_folder', (req, res) => {
    ...
    const userid = req.body.folder_name;
    let i = 0;
    connection.query(`select * from folders`, (err, rows) => {
        if (err) {
            throw err;
            ...
            console.log("Name not exist now we can add");
            connection.query(
                `insert into folders (name,id) values ('${folderName}','${userid}')`,
                (err, result) => {
                    if (err) {
                        console.log(err);
                        res.send("Error creating folder");
        ...
        if (err) {
            ...
            for (i = 0; i < rows.length; i++) {
                if (rows[i].username == Newname) {
                    if (rows[i].email_address != Newaddress) {
                        res.status(200).send("You cannot change the email");
                        console.log(rows.length);
                        return;
                    }
                    res.status(200).send("This name already Exist");
                    console.log(rows.length);
                    return;
                }
            }
