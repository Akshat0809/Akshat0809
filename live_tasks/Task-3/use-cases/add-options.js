...
module.exports = function makeproductoption({
    ...
    return async function addProductoption(product_id, obj) {
        console.log("in use case of create option");
        var arr = [];
        for (let i of obj) {
            let totalVariants = i.Variants;
            if (typeof (totalVariants) === 'string') {
                let Variants = (totalVariants).split(';');
